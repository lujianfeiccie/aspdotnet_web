...
namespace DAL
{
    ...
    public class tb_sfjc
    {
        ...
        public int GetMaxId()
        {
            return DbHelperSQL.GetMaxID("ID", "tb_sfjc");
        }
        ...
        public bool Exists(int ID)
        {
            ...
            strSql.Append("select count(1) from tb_sfjc");
        ...
        public int Add(Model.tb_sfjc model)
        {
            ...
            parameters[27].Value = model.Id_card;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update tb_sfjc set ");
            ...
            parameters[28].Value = model.Id_card;
        ...
        public Model.tb_sfjc GetModel(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ID,DirectReportNumber,ConfirmationNumber,ConfirmationDate,Name,FK_Sex,BirthDate,FK_InfectionWay,FK_CurrentSituation,FK_District,FK_TypeOfTreatment,AddingTime,TreatmentNumber,FK_MaintainStatus,FK_FirstFollowUpVisit,FK_FollowUpIntervention,CD4,ViralLoad,FK_Spouse_AntibodyDetection,FK_DetectBefore,FK_Children_AntibodyDetection,FK_IsDead,Remark,FK_User,SendingTime,FK_Flag,showToUser,VerifyDate,Id_Card from tb_sfjc ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;
        ...
        public DataSet GetDetailedViewAllList()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM v_DetailedSfjc ");
            return DbHelperSQL.Query(strSql.ToString());
        }
        /// <summary>
        /// 获得视图数据列表通过ID
        ...
        public DataSet GetViewList(int StartIndex, int EndIndex, string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" Select * ");
            strSql.Append(" FROM (SELECT ROW_NUMBER() OVER(Order by SendingTime DESC) AS rownum,* FROM v_Sfjc ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" Where " + strWhere);
            }
        ...
        public int AddBatchData(DataSet ds)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into tb_sfjc(");
            strSql.Append("Name,DirectReportNumber,ConfirmationNumber,ConfirmationDate,FK_Sex,BirthDate,FK_InfectionWay,FK_CurrentSituation,FK_TypeOfTreatment,FK_District,CD4,ViralLoad,FK_Spouse_AntibodyDetection,FK_Children_AntibodyDetection,Remark)");
            strSql.Append(" values (");
            strSql.Append("@Name,@DirectReportNumber,@ConfirmationNumber,@ConfirmationDate,@FK_Sex,@BirthDate,@FK_InfectionWay,@FK_CurrentSituation,@FK_TypeOfTreatment,@FK_District,@CD4,@ViralLoad,@FK_Spouse_AntibodyDetection,@FK_Children_AntibodyDetection,@Remark)");
            strSql.Append(";select @@IDENTITY");
            ...
            for (i = 0; i < count; i++)
            {
                SqlParameter[] parameters = {
					...
					new SqlParameter("@FK_Children_AntibodyDetection", SqlDbType.Int,4),
					new SqlParameter("@Remark", SqlDbType.NVarChar)};
                parameters[0].Value = ds.Tables[0].Rows[i][0];
                String DirectReportNumber = ds.Tables[0].Rows[i][1].ToString();
                parameters[1].Value = DirectReportNumber.Substring(0, 4) + "-" + DirectReportNumber.Substring(4, 5) + "-" + DirectReportNumber.Substring(9,4) + "-" + DirectReportNumber.Substring(13, 5); 
                parameters[2].Value = "sz" + ds.Tables[0].Rows[i][2].ToString();
