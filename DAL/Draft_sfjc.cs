...
namespace DAL
{
    ...
    public class Draft_sfjc
    {
        ...
        public int GetMaxId()
        {
            return DbHelperSQL.GetMaxID("ID", "Draft_sfjc");
        }
        ...
        public bool Exists(int ID)
        {
            ...
            strSql.Append("select count(1) from Draft_sfjc");
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Model.Draft_sfjc model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into Draft_sfjc(");
            strSql.Append("DirectReportNumber,ConfirmationNumber,ConfirmationDate,Name,FK_Sex,BirthDate,FK_InfectionWay,FK_CurrentSituation,FK_District,FK_TypeOfTreatment,AddingTime,TreatmentNumber,FK_MaintainStatus,FK_FirstFollowUpVisit,FK_FollowUpIntervention,CD4,ViralLoad,FK_Spouse_AntibodyDetection,FK_DetectBefore,FK_Children_AntibodyDetection,FK_IsDead,Remark,FK_User,SendingTime,FK_Flag,showToUser,VerifyDate,Id_Card,Operator_id)");
            ...
            parameters[28].Value = model.Operator_id;
        ...
        /// 更新一条数据
        /// </summary>
        public void Update(Model.Draft_sfjc model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update Draft_sfjc set ");
        ...
        public void Delete(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete Draft_sfjc ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.Draft_sfjc GetModel(int ID)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ID,DirectReportNumber,ConfirmationNumber,ConfirmationDate,Name,FK_Sex,BirthDate,FK_InfectionWay,FK_CurrentSituation,FK_District,FK_TypeOfTreatment,AddingTime,TreatmentNumber,FK_MaintainStatus,FK_FirstFollowUpVisit,FK_FollowUpIntervention,CD4,ViralLoad,FK_Spouse_AntibodyDetection,FK_DetectBefore,FK_Children_AntibodyDetection,FK_IsDead,Remark,FK_User,SendingTime,FK_Flag,showToUser,VerifyDate,Id_Card,Operator_id from Draft_sfjc ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;

            Model.Draft_sfjc model = new Model.Draft_sfjc();
        ...
        public DataSet GetViewList(int StartIndex, int EndIndex, string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" Select * ");
            strSql.Append(" FROM (SELECT ROW_NUMBER() OVER(Order by SendingTime DESC) AS rownum,* FROM Draft_sfjc ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" Where " + strWhere);
            }
