...
namespace DAL
{
    ...
    public class temp_tb_lxbx
    {
        ...
        public int GetMaxId()
        {
            return DbHelperSQL.GetMaxID("ID", "temp_tb_lxbx");
        }
        ...
        public bool Exists(int ID)
        {
            ...
            strSql.Append("select count(1) from temp_tb_lxbx");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into temp_tb_lxbx(");
            strSql.Append("ConfirmationNumber,DirectReportNumber,Name,FK_Sex,BirthDate,FK_Nationality,FK_MarriageStatus,FK_Education,FK_Citizenship,Province,County,FK_InfectionWay,FK_ContactHistory,FK_JobCharacter,FK_DetectionClass,FK_HouseholdRegister,FK_CurrentSituation,PositiveDecisionDate,ToAidsDate,FK_District,DetailedAddress,CensorshipUnit,FK_DetectionStatement,Spouse,Children,FK_IsDead,Remark,FK_User,SendingTime,FK_Flag,FK_ShowToUser,VerifyDate,FK_Spouse,FK_Children)");
            ...
            parameters[32].Value = model.FK_Spouse;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update temp_tb_lxbx set ");
            ...
            parameters[34].Value = model.FK_Children;
        ...
        public void Delete(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete temp_tb_lxbx ");
            strSql.Append(" where ID=@ID ");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            strSql.Append(" FROM temp_tb_lxbx ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetViewAllList()
        {
            ...
            strSql.Append(" FROM v_Temp_Lxbx ");
            return DbHelperSQL.Query(strSql.ToString());
        }
        /// <summary>
        /// 获得视图数据列表
        ...
        public DataSet GetDetailedViewAllList()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM v_DetailedTemp_Lxbx ");
            return DbHelperSQL.Query(strSql.ToString());
        }
        /// <summary>
        /// 获得视图数据列表通过ID
        ...
        public DataSet GetViewList(int StartIndex, int EndIndex, string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" Select * ");
            strSql.Append(" FROM (SELECT ROW_NUMBER() OVER(Order by SendingTime DESC) AS rownum,* FROM v_DetailedTemp_Lxbx ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" Where " + strWhere);
            }
