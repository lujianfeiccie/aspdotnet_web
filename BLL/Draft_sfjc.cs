...
{
    /// <summary>
    /// 业务逻辑类Draft_sfjc 的摘要说明。
    /// </summary>
    public class Draft_sfjc
    {
        private readonly DAL.Draft_sfjc dal = new DAL.Draft_sfjc();
        public Draft_sfjc()
        { }
        #region  成员方法
        ...
        /// 得到一个对象实体，从缓存中。
        /// </summary>
        public Model.Draft_sfjc GetModelByCache(int ID)
        {

            string CacheKey = "Draft_sfjcModel-" + ID;
            ...
            return (Model.Draft_sfjc)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.Draft_sfjc> GetModelList(string strWhere)
        {
            DataSet ds = dal.GetList(strWhere);
            List<Model.Draft_sfjc> modelList = new List<Model.Draft_sfjc>();
            int rowsCount = ds.Tables[0].Rows.Count;
            if (rowsCount > 0)
            {
                Model.Draft_sfjc model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Model.Draft_sfjc();
                    ...
                    //{
                    //    model.showToUser = int.Parse(ds.Tables[0].Rows[n]["Operator_id"].ToString());
                    //}
